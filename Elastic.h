...
void Elastic::calculateStressOneDimensional(Array2D<double>& strain, Array2D<double>& dstrain, Array2D<double>& stress, double timestep){
	strain += dstrain;
	stress[0][0] = strain[0][0] * youngs_moduls;
}

void Elastic::calculateStressTwoDimensional(Array2D<double>& strain, Array2D<double>& dstrain, Array2D<double>& stress, double timestep){
	...
	dstrain[2][0] = -(dstrain[1][0] + dstrain[0][0])*(nu / (1 - nu));//如果是塑性材料，就要在应力达到屈服应力，也就是曲阜函数判据，后，以试探法计算塑性应变增量，塑性应变，通过屈服函数得到应力；而厚度方向应变=dstrain[1][0] - dstrain[0][0]
	//update strain
	strain += dstrain;
}

void Elastic::calculateStressThreeDimensional(Array2D<double>& strain, Array2D<double>& dstrain, Array2D<double>& stress, double timestep){
	strain += dstrain;
