...
public:
	...
	int parseNodeVectorAndTable(vector<Node>& nodeList, map<int, Node>& nodeTable, vector<BdCondition>& constraintList,vector<Rigid_Body> &rigidConstraintList, vector<Load>& loadList);
	...
	int parseElementOfSolid_Iso_6(vector<Elastic>& materialList, vector<Node>& nodeList, vector<Load> loadList, map<int, Node>& nodeTable, vector<Solid_Iso_6>& elementList, map<int, Solid_Iso_6>& elementTable);

	int parseLoad(vector<Load>& loadList);

	int parseConstraint_BdCondition(vector<BdCondition>& constraintList);
	int parseConstraint_RigidCondition(vector<Rigid_Body>& constraintList);
...
int Fembic::parseConstraint_BdCondition(vector<BdCondition>& constraintList){
	...
	while (getline(file, thisLine))
	{
		...
		if (in_block)
		{
			...
			/* 此处应该利用类工厂，生成一个对应的类，用来调用parse                                                                     */
			/************************************************************************/
			if (current_constraint_type=="BOUNDARY_CONDITION")
			{
				BdCondition BCconstraint;
				BCconstraint.parseFembic(thisLine);
				BCconstraint.x_vel.printV();
				cout<<"约束名字为" << BCconstraint.name << endl;
				constraintList.push_back(BCconstraint);
			}
			else{
				cout << "BCondition约到了Rigid，当然退出啦" << endl;
			}
...
int Fembic::parseConstraint_RigidCondition(vector<Rigid_Body>& constraintList){
	string fileName = filePath;
	ifstream file;//#include <fstream>
	file.open(fileName);
	if (!file.is_open()){
	...
	while (getline(file, thisLine))
	{
		...
		if (in_block)
		{
			...
			if (current_constraint_type == "BOUNDARY_CONDITION")
			{
				cout << "Rigid约到了BCondition，也退出啦" << endl;
			}
			else{

				Rigid_Body Rigidconstraint;
				Rigidconstraint.parseFembic(thisLine);
				cout << "约束名字为" << Rigidconstraint.name << endl;
				constraintList.push_back(Rigidconstraint);
			}
...
int Fembic::parseElementOfSolid_Iso_6(vector<Elastic>& materialList, vector<Node>& nodeList, vector<Load> loadList, map<int, Node>& nodeTable, vector<Solid_Iso_6>& elementList, map<int, Solid_Iso_6>& elementTable){
	...
	while (getline(file, thisLine))
	{
		...
		if (in_block)
		{
			...
			//temporary_element = new Rod_2();
			++numberOfElements;
			temporary_element.element_ID = numberOfElements;
			temporary_element.parse_Fembic(thisLine, materialList, loadList, nodeList, nodeTable);
...
int Fembic::parseNodeVectorAndTable(vector<Node>& nodeList, map<int, Node>& nodeTable, vector<BdCondition>& constraintList, vector<Rigid_Body> &rigidConstraintList, vector<Load>& loadList){
		...
		if (in_block)//a wonderful design
		{
			...
			for (position; position != tok.end(); position++){
				...
				if (tempString == "X"){
					...
					tempString = *position;
					transform(tempString.begin(), tempString.end(), tempString.begin(), toupper);
					for (auto temp_constraint1 : constraintList)
					{
						if (temp_constraint1.name == (tempString))
						{
							/************************************************************************/
							/* 需要重构                                                                     */
							/************************************************************************/
							temporaryNode.constraint = temp_constraint1;
							temporaryNode.constraint_is_set = true;
							temporaryNode.BdCondition_is_set = true;
						}
					}
					for (auto temp_constraint2:rigidConstraintList)
					{
						if (temp_constraint2.name == (tempString))
						{
							/************************************************************************/
							/* 需要重构                                                                     */
							/************************************************************************/
							temporaryNode.rigidConstraint = temp_constraint2;
							temporaryNode.constraint_is_set = true;
							temporaryNode.rigidCondition_is_set = true;
							temp_constraint2.registerNode(temporaryNode);
						}
					}
