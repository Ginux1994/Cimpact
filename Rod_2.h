...
public://just for easy
	...
	Array2D<double> dstrain, strain, stress, force, global_force;
...
Rod_2::Rod_2(){
	...
	stress = Array2D<double>(6, 1, 0.0);
	strain = Array2D<double>(6, 1, 0.0);
	dstrain = Array2D<double>(6, 1, 0.0);
...
double Rod_2::checkTimeStep(double current_timestep){
	...
	timestep = critical_length / material.wavespeedOneDimendinal(0.0, 0.0);
...
void Rod_2::checkIfFailed(){
	...
	{
		if (stress[0][0]>material.failure_strss)
		{
			failed = true;
		}
	}
	...
	{
		if (strain[0][0]>material.failure_strain)
		{
			failed = true;
			return;
		}
...
void Rod_2::calculateNodalForces(double timestep, int integration_points){
	force[0][0] = stress[0][0] * cross_section_area;
