...
ModelSmp::ModelSmp(int nCPUs, string& fileName){
	...
	/*            //解析约束集                                                          */
	/************************************************************************/
	indataFile.parseConstraint(constraintList);
	...
	/************************************************************************/
	indataFile.parseNodeVectorAndTable(nodeList, nodeTable, constraintList, loadList);
...
void ModelSmp::setInitialContion(){
	...
	for (auto tempNode : nodeList){
		if (tempNode.constraint_is_set){
			tempNode.applyAccelerationCondition(0);
			tempNode.applyVelocityCondition(0);
		}
		else{
			tempNode.vel = 0;
			tempNode.acc = 0;
...
//		cout << "3号结点Z位置 \n";
		cout_dpl << currtime << "\t" << elementTable[1].stress[0][0] << "\n";
