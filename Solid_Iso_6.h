...
public:
	...
	void checkIfFailed();
	void updateLocalBaseVector();//局部坐标系的更新是为了将结点力转化到整体坐标系中，但实体单元中力的计算是在整体坐标系，不需要转换
...
void Solid_Iso_6::calculateMassMatrix(){
	...
	/************************************************************************/
	/*质量阵计算有问题，按照WK-LIU书50页，质量阵是ρ*N.transpose()*N*J*w(i,j,k)  
	也就是说，少乘了一个N*/
	/************************************************************************/
...
void Solid_Iso_6::calculateStrain(double step, int k){
	//每个积分点权系数都是+1/√3或-1/√3，注意区分积分点和自然坐标，计算D、N的时候用的是积分点，
	//计算B阵中的坐标阵M以及H阵为笛卡尔坐标、自然坐标
