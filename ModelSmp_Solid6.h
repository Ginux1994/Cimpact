#ifndef MODELSMP_SOLID6_H
#define MODELSMP_SOLID6_H

#include "allInclude.h"

using namespace std;
class ModelSmp_Solid6
{
public:

	ModelSmp_Solid6(int, string& );

	void migrate(map<int, Solid_Iso_6>& elementTable, vector<Node>& nodeList);
...
//public for easy	
	...
	vector<Solid_Iso_6> elementList;//单元向量

	map<int, Solid_Iso_6> elementTable;//单元映射表

	vector<Elastic> materialList;//材料向量

	vector<BdCondition> constraintList;//约束向
...
ModelSmp_Solid6::ModelSmp_Solid6(int nCPUs, string& fileName){
	using namespace std;
	nr_of_CPUs = nCPUs;
	filePath = fileName;
	...
	indataFile.parseElementOfSolid_Iso_6(materialList, nodeList, loadList, nodeTable, elementList, elementTable);//解析单元集，并将材料节点集放入相应单元的成员变量中，形成单元向量，单元映射表
	//至此，输入文件中的所有信息都有了，可以直接用于计算
	cout << "解析文件并处理节点单元后的结果(=====单元列=====) \n";

	for (auto testElement : elementList)
	{
		cout << "单元编号为 " << testElement.element_ID << " 的单元，其材料信息为 \n";
		cout << "杨氏模量： " << testElement.material[0].youngs_moduls << endl;
		cout << "密度： " << testElement.material[0].density << endl;
	}


	cout << "\n\n\n\n\n";
	cout << "解析文件并处理节点单元后的结果(=====单元表=====) \n";
	auto testElement = elementTable[2];
	cout << "单元编号为 " << testElement.element_ID << " 的单元，其材料信息为 \n";
	cout << "杨氏模量： " << testElement.material[0].youngs_moduls << endl;
	cout << "密度： " << testElement.material[0].density << endl;
...
void ModelSmp_Solid6::assembleMassMatrix(){
	cout << "=====华丽丽的分割线  之  计算出单元在结点上的分布质量值，并同步单元表单元列===\n";
	for (int i = 1; i <= elementTable.size(); i++)
	{
		elementTable[i].calculateMassMatrix();//通过集中质量法，将质量分布集中在节点nodeTable上，同时也集中到了单元的左右节点上
	...
	for (int i = 1; i <= elementTable.size(); i++){
		for (int j = 0; j < 8;j++)
		{
			elementTable[i].nodes[j].determineInertiaMatrix();
		}
...
void ModelSmp_Solid6::setInitialContion(){
	...
	for (auto element : elementList)
	{
		...
		cout << "=====华丽丽分割线 之 单元刚阵：=====\n";
		cout << "单元刚阵存储在单元的材料中; 三维刚度阵" << element.material[0].stiffness_matrix_3d << endl;
		cout <<"平面应力问题的刚度阵 ："<< element.material[0].stiffness_matrix_plane_stress << endl;
	}
	...
	if (true)
	{
		...
		{
			cout << "第 " << tempElement.element_ID << " 号单元的质量为 " << tempElement.total_mass << endl;
		}
	}
...
void ModelSmp_Solid6::solve(){

	long time_info;
	long time_info_tmp;
	long time_remained;
	int time_h, time_m, time_s;//used for time show
	int number_of_integration_points = 1;//1 for Solid_Iso_6 element
	string time_str;
	bool autosave = false;
	...
	for (currtime = controlset.startTime; currtime <= controlset.endTime; currtime += timestep)
	{
		...
		for (auto temporary_element:elementList)
		{
			if (!temporary_element.isDeactivated())//失效的检查放在后面，毕竟最开始是坑定不失效的
			{
				...
				//计算外部力，如重力体力，其他
				temporary_element.calculateExternalForces();
		...
		{
			Node *master_node = &nodeTable[temp_constraint.master_node_number];
//			temp_constraint.update();//本质上对于BDcondition更新的是局部坐标系，但对于rigid，更新的是力矩
			for (auto node : temp_constraint.nodes){
				//获取节点力
				temp_constraint.force = node.force.subarray(0, 2);
				//累加到主节点上
				master_node->addInternalForce(temp_constraint.force);
				//获取主节点力矩
				temp_constraint.moment = node.force.subarray(3, 5);
				//计算从节点到主节点的力臂
				temp_constraint.moment_arm = node.pos - master_node->pos;
				//力臂×力 = 力矩
				temp_constraint.moment_arm = temp_constraint.moment_arm.crossProduct(temp_constraint.force);
				temp_constraint.moment += temp_constraint.moment_arm;
				//累计到主节点上
				master_node->addInternalMoment(temp_constraint.moment);
			}
		...
		for (auto temp_element : elementList){
			for (int i = 0; i < number_of_integration_points;i++)
			{
				temp_element.nodes[i].calculateNewPosition(timestep, currtime);
				temp_element.nodes[i].checkNeighbours();
			}
		}
		if (DEBUG)
		{
			cout << "************************华丽丽分割线 之 节点位移*******************************\n";
			cout << "=========================一号单元 左右节点X方向位移==========================================\n";
			cout << "左节点X位移为 = " << elementTable[10].nodes[3].dpl[0] << endl;
			cout << "右节点X位移为 = " << elementTable[10].nodes[3].dpl[0] << endl;
			cout << "************************华丽丽分割线 之 写出位移应力结果到文件中*******************************\n";

		}
		//这部分可以封装在Track中，根据需要输出相应值
//		cout << "3号结点Z位置 \n";
		cout_dpl << currtime << "\t" << elementTable[2].stress[0] << "\n";
...
/************************************************************************/
/* 
void ModelSmp_Solid6::migrate(map<int, Solid_Iso_6>& elementTable, vector<Node>& nodeList){
...
void ModelSmp_Solid6::migrate(map<int, Node>& nodeTable, vector<Node>& nodeList){
	nodeList.clear();
